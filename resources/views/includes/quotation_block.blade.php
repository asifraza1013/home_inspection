        ...
        <div class="input-group h-100">
            <select name="company" id="select-company" class="form-control">
                @foreach ($compnies as $key => $company)
                    <option co-key="{{ $key }}" value="{{ $company->id }}">{{ $company->company_name }}
                    </option>
                @endforeach
            </select>
        </div>
...
<div class="row company-detail-block">
    @foreach ($compnies[0]->pricing['item_name'] as $key => $item)
        <div class="col-lg-6">
        ...
        <div class="col-lg-6 mt-1">
            <div class="input-group h-100">
                <input type="text" class="form-control" name="" placeholder=""
                    value="{{ $compnies[0]->pricing['item_price'][$key] }}" readonly>
                <span class="my-auto ml-2" style="margin-left: 10px"><input type="checkbox"
                        class="form-check-input pricing-checkbox"
                        check-price="{{ $compnies[0]->pricing['item_price'][$key] }}" name="item_selection[]"
                        {{ in_array($item, $compnies[0]->pricing['item_selection']) ? 'checked disabled' : null }}
                        value="{{ $item }}"></span>
            </div>
        </div>
    @endforeach
...
<script>
    $(document).ready(function() {
        ...
        let selectedCompany = 0; // bydefault first company will be selected
                ...
                html +=    '<div class="col-lg-6">\
                    ...
                    <div class="col-lg-6 mt-1">\
                        <div class="input-group h-100">\
                            ...
                            <span class="my-auto ml-2" style="margin-left: 10px"><input type="checkbox" class="form-check-input pricing-checkbox" check-price=" '+allCompanies[selectedCompany].pricing['item_price'][index]+'" name="item_selection[]" '+(jQuery.inArray(values, allCompanies[selectedCompany].pricing['item_selection']) !== 1) ? "checked disblaed " : null+' value="'+values+'"></span>\
                        </div>\
                    </div>'
            });
